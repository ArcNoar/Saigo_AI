...
class Node_Manager():
    ...
    def node_generator(self,Node_Object='Abstract'): # Creates Node for new data
        ...
        if Node_Object == 'Abstract':
            ...
            UNIC_ID = randint(0,100000)

            # We don't know how to adress to Abstraction, need to fix
            
            if faint_Node == True:
                Node_Class = 0
                self.NB[f'{Node_Object} | - {UNIC_ID}'] = [round(self.NCV,3),Node_Class,1] 
            else:
                Node_Class = 2
                self.NB[f'{Node_Object} | - {UNIC_ID}'] = [round(self.NCV,3),Node_Class,1] 
    ...
    def Node_Proc(self):
        ...
        for i in self.TP:
            ...
            # We are not cheking Abstract Cases (SHOULD FIX)
